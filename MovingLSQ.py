...
def Error_2D_Affine(x0, src, dst, weight_map, p_star, q_star, buf, buf_map):
...
def Error_2D_Rigid(x0, src, dst, weight_map, p_star, q_star, buf, buf_map):
    label_num = src.shape[0]
    a_cos = np.cos(x0)
    a_sin = np.sin(x0)
...
class MovingLSQ:
    ...
    def Run_Affine(self, srcPts, alpha = 1):
        ...
        result = least_squares(Error_2D_Affine, x0, jac_sparsity = jacobian, verbose=2,
    ...
    def Run_Rigid(self, srcPts, alpha = 1):
        ...
        #exit()
        result = least_squares(Error_2D_Rigid, x0, jac_sparsity = jacobian, verbose=2, bounds=(lower_bound, upper_bound),
                args=(self._src, self._dst, weight, p_star, q_star, buf, buf_map))
